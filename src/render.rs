...
impl Renderer {
    ...
    pub fn render_word(&self, word: &str, canvas_w: u32, canvas_h: u32, color: (u8, u8, u8)) -> Vec<u8> {
        ...
        // Center the word vertically and horizontally
        ...
        for (metrics, bitmap) in &glyphs {
            ...
            for row in 0..metrics.height {
                for col in 0..metrics.width {
                    ...
                    // Mix with the color. Coverage is the alpha/intensity.
                    canvas[idx] = color.0;     // R
                    canvas[idx + 1] = color.1; // G
                    canvas[idx + 2] = color.2; // B
                    canvas[idx + 3] = coverage; // Use coverage as alpha
                }
            }
...
/// Binary-search for the largest font size where the word fits within the canvas.
...
fn measure(font: &Font, word: &str, size: f32) -> (f32, f32) {
    ...
    let max_asc = metrics
        .iter()
        .map(|m| (m.ymin + m.height as i32).max(0))
        .max()
        .unwrap_or(0) as f32;
    let max_desc = metrics
        .iter()
        .map(|m| (-m.ymin).max(0))
        .max()
        .unwrap_or(0) as f32;
    (total_w, max_asc + max_desc)
}
